...
@section('content')
    ...
    <section class="bg-indigo-900 text-white relative overflow-hidden">
        ...
        <div class="max-w-6xl mx-auto px-5 py-14 relative">
            ...
            <!-- search -->
            <div class="mt-8 flex gap-3 max-w-md">
                <div class="relative flex-1">
                    ...
                    <input id="searchInput" type="text" placeholder="Cari nama atau jabatan..."
                        class="w-full pl-9 pr-4 py-3 rounded-xl bg-indigo-800 border border-indigo-700 text-white placeholder-indigo-400 text-sm" />
                </div>
...
<section class="max-w-6xl mx-auto px-5 py-10">
    ...
    {{-- Filter Tab --}}
    <div class="flex gap-2 flex-wrap mb-6" id="filterBar">
        <button onclick="setFilter('semua', this)"
            class="filter-btn active px-4 py-2 rounded-full text-sm font-medium border border-indigo-200 bg-indigo-700 text-white transition-all">
            Semua (16)
        </button>
        <button onclick="setFilter('guru', this)"
            class="filter-btn px-4 py-2 rounded-full text-sm font-medium border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 transition-all">
            Guru (12)
        </button>
        <button onclick="setFilter('staf', this)"
            class="filter-btn px-4 py-2 rounded-full text-sm font-medium border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 transition-all">
            Staf TU (4)
        </button>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4" id="staffGrid">
        @foreach ($pegawai as $p)
            ...
            @endphp
            <div class="staff-card bg-white border border-gray-100 rounded-2xl p-5 flex flex-col items-center text-center gap-3 relative hover:-translate-y-1 hover:shadow-md transition-all cursor-pointer"
                 data-tipe="{{ $p->tipe }}"
                 data-nama="{{ strtolower($p->nama) }}"
                 data-jabatan="{{ strtolower($p->jabatan) }}"
                 onclick="window.location='{{ route('pegawai.show', $p->id) }}'">

                {{-- Status dot --}}
                <span class="absolute top-3 right-3 w-2 h-2 rounded-full bg-emerald-400" title="Aktif"></span>

                {{-- Avatar --}}
                <div class="w-14 h-14 rounded-full {{ $avatarBg }} flex items-center justify-content-center font-semibold text-lg">
                    {{ $initials }}
                </div>
                ...
                <span class="text-[11px] text-gray-500 bg-gray-50 px-2 py-1 rounded-lg mt-auto">
                    <svg class="inline w-3 h-3 mr-0.5 -mt-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 6V4m0 2a2 2 0 1 0 0 4m0-4a2 2 0 1 1 0 4m-6 8a2 2 0 1 0 0-4m0 4a2 2 0 1 1 0-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 1 0 0-4m0 4a2 2 0 1 1 0-4m0 4v2m0-6V4"/></svg>
                    {{ $p->mapel ?? $p->bidang }}
                </span>
            </div>
        @endforeach
...
<script>
    ...
    function filterCards() {
        ...
        document.querySelectorAll('.staff-card').forEach(card => {
            ...
            const matchSearch = !query || card.dataset.nama.includes(query) || card.dataset.jabatan.includes(query);
