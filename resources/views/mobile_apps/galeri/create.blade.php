...
@section('content')
    <div class="px-5 pt-4">
        ...
        <div class="flex items-center gap-3 mb-5">
            ...
            <h2 class="font-sora font-bold text-slate-800 text-base">Unggah Galeri Baru</h2>
        </div>

        <!-- Form Card -->
        <div class="bg-white border border-slate-100/80 shadow-xs rounded-2xl p-5 mb-6" x-data="galleryUpload()">
            <form id="gallery-create-form" action="{{ route('apps.galeri.store') }}" method="POST" enctype="multipart/form-data" @submit="handleSubmit($event)">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Judul Kegiatan</label>
                        <input type="text" name="judul" value="{{ old('judul') }}" required placeholder="Contoh: Kemah Bakti Pramuka 2026"
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="3" required placeholder="Tulis rincian singkat kegiatan..."
                                  class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-xs text-slate-800 focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/10 transition-all font-semibold">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="text-rose-500 text-[10px] mt-1 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="kategori" required
                                ...
                                <option value="Pramuka" {{ old('kategori') == 'Pramuka' ? 'selected' : '' }}>Pramuka</option>
                        ...
                        <div>
                            ...
                            <input type="number" name="tahun" value="{{ old('tahun', date('Y')) }}" required min="2020" max="{{ date('Y') + 1 }}"
                                   class="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-2.5 text-xs text-slate-800 focus:outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/10 transition-all font-semibold">
                            @error('tahun')
                                <p class="text-rose-500 text-[10px] mt-1 font-semibold">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1.5">Unggah Foto Kegiatan</label>
                        ...
                        <div class="relative group"
                            ...
                            <div class="border-2 border-dashed rounded-2xl p-6 text-center transition-all duration-300"
                                ...
                                <p class="text-xs text-slate-500 font-bold" x-text="dragOver ? 'Lepaskan foto di sini...' : 'Pilih foto kegiatan'"></p>
                                <p class="text-[10px] text-slate-400 mt-1">Dapat memilih lebih dari satu. Maksimal 2MB per foto.</p>
                            </div>
                        </div>
                        @error('photos')
                            <p class="text-rose-500 text-[10px] mt-1 font-semibold">{{ $message }}</p>
                        ...
                        <div class="mt-4 space-y-3" x-show="selectedFiles.length > 0" style="display: none;">
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Foto Terpilih (<span x-text="selectedFiles.length"></span>)</p>
                    ...
                    <button type="submit" id="gallery-submit-btn" :disabled="submitting || selectedFiles.length === 0"
                        ...
                        <span x-text="submitting ? 'Mengirim & Mengunggah...' : 'Kirim Usulan Galeri'"></span>
                    </button>
                </div>
            </form>
        </div>
            ...
            return {
                ...
                handleSubmit(e) {
                    ...
                    if (window.showGlobalLoader) {
                        window.showGlobalLoader('Mengunggah Galeri...', 'Sedang menyimpan data dan mengunggah gambar Anda ke server');
                    }
                }
            }
        }
