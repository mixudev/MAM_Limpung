...
<head>
    ...
    <title>Masuk dengan OTP — {{ config('app.name', 'MAM Limpung') }}</title>
...
<div class="w-full max-w-4xl bg-white shadow-2xl flex flex-col md:flex-row h-full max-h-[600px] border border-slate-200">
    ...
    <div class="hidden md:block md:w-1/2 relative overflow-hidden bg-slate-900">
        ...
        <div class="absolute bottom-10 left-10 z-20">
            ...
            <h2 class="text-3xl font-black text-white uppercase tracking-tighter leading-tight mb-2 drop-shadow-lg">Login<br>OTP</h2>
            <p class="text-slate-200 text-xs font-bold tracking-widest uppercase drop-shadow-md">Masuk Tanpa Kata Sandi</p>
        </div>
    </div>

    <!-- Right Side: Request OTP Form -->
    <div class="w-full md:w-1/2 p-8 md:p-10 flex flex-col justify-center h-full bg-white relative">
        
        <div class="my-6">
            <h3 class="text-2xl font-black uppercase tracking-tighter text-slate-900 mb-1">Masuk dengan OTP</h3>
            <p class="text-xs text-slate-500 font-medium">Masukkan email terdaftar Anda. Kami akan mengirimkan 6-digit kode OTP.</p>
        </div>
        ...
        <form method="POST" action="{{ route('login.otp.send') }}" class="space-y-6">
            ...
            <!-- Email Address -->
            <div class="my-5">
                <label for="email" class="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Alamat Email Terdaftar</label>
                <div class="relative">
        ...
        <div class="mt-auto pt-4 text-center border-t border-slate-100 mt-6">
            <a href="{{ route('password.request') }}" class="inline-flex items-center text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-blue-900 transition-colors">
                Lupa Kata Sandi? Reset Melalui Link Email
            </a>
        </div>
