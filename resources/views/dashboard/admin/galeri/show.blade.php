...
<button 
    ...
    class="py-2 px-4 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-none transition-all tracking-wider font-mono">
    TOLAK
</button>
...
<!-- Modal Reject (tanpa Alpine) -->
<div id="reject-modal" class="fixed inset-0 z-50 items-center justify-center bg-black/60 backdrop-blur-sm p-4" style="display: none;">
    <div class="bg-white dark:bg-zinc-900 border border-slate-250 dark:border-zinc-800 w-full max-w-md p-6 shadow-2xl relative">
        ...
        <form action="{{ route('admin.galeri.reject', $galeri->uuid) }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">
            ...
            <div class="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-zinc-800">
                <button 
                    ...
                    class="py-2 px-4 bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 text-slate-700 dark:text-zinc-300 font-bold text-xs rounded-none transition-all font-mono">
                    BATAL
                </button>
    ...
    <div class="absolute inset-0 -z-10" onclick="document.getElementById('reject-modal').style.display = 'none'"></div>
</div>
</div>
@endsection
