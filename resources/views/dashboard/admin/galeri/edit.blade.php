...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (breadcrumb) {
            breadcrumb.textContent = 'Edit Galeri Foto';
        }
    });
</script>

<div class="max-w-4xl space-y-6" x-data="galeriEditForm()">
    <!-- Header -->
    <div class="bg-white dark:bg-zinc-900 p-6 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Edit Album Galeri</h1>
            <p class="text-xs text-slate-500 dark:text-zinc-400 mt-1">Perbarui judul, deskripsi, tambah foto baru, hapus foto yang tidak diinginkan, atau ubah foto sampul utama.</p>
        </div>
    ...
    <!-- Alert for Student Status Resets -->
    @if(!Auth::user()->hasRole('admin') && !Auth::user()->hasRole('super-admin'))
        <div class="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/65 p-4 text-amber-800 dark:text-amber-400 text-xs font-semibold rounded-none flex items-start gap-2">
            <svg class="w-4 h-4 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <div>
                <span class="font-bold">Perhatian:</span> Karena Anda masuk sebagai Siswa/Guru, menyimpan perubahan pada album ini akan mengubah statusnya kembali menjadi <span class="underline">Pending</span> dan menyembunyikannya dari galeri umum sampai disetujui kembali oleh Admin.
            </div>
        </div>
    @endif

    <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm p-6">
        <form action="{{ route('admin.galeri.update', $galeri->uuid) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
                ...
                <div class="md:col-span-3 space-y-2">
                    <label class="block text-xs font-mono font-bold uppercase tracking-wider text-slate-450 dark:text-zinc-400">Tambah Berkas Gambar Baru</label>
                    <div class="flex items-center justify-center w-full">
                        ...
                        >
                            <div class="flex flex-col items-center justify-center pt-4 pb-5 text-center px-4">
                                ...
                                <p class="text-xs text-slate-600 dark:text-zinc-400"><span class="text-[#4f45b2] font-semibold underline">Pilih foto-foto baru</span> atau seret foto ke sini</p>
                ...
                <div class="md:col-span-2 space-y-2">
                    <div class="flex justify-between items-center">
                        <label class="block text-xs font-mono font-bold uppercase tracking-wider text-slate-450 dark:text-zinc-400">Tambah Tautan Gambar Eksternal Baru</label>
                        ...
                        <template x-for="(link, index) in links" :key="index">
                            <div class="flex gap-2 items-center">
                                <input type="url" name="links[]" x-model="link.url" placeholder="https://example.com/gambar.jpg"
                                    class="flex-1 px-3 py-1.5 text-xs bg-white dark:bg-zinc-855 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]" />
            ...
            <div class="border-t border-slate-100 dark:border-zinc-800 pt-6 space-y-3">
                <h3 class="text-xs font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Kelola Media & Penentuan Sampul Utama (Klik Bintang)</h3>
                
                <!-- Hidden inputs for existing photos keeping -->
                <template x-for="photo in existingPhotos" :key="'keep-'+photo.id">
                ...
                <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
                    ...
                    <!-- Existing Photos (Database) -->
                    <template x-for="(photo, index) in existingPhotos" :key="'exist-'+photo.id">
                        <div class="relative aspect-square border border-slate-200 dark:border-zinc-800 p-1 bg-slate-50 dark:bg-zinc-955 flex flex-col justify-between group overflow-hidden">
                            <div class="w-full h-full relative overflow-hidden bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800">
                                <img :src="photo.tipe === 'link' ? photo.file_path : '{{ asset('storage') }}/' + photo.file_path" class="absolute inset-0 w-full h-full object-cover">
                                
                                <!-- Cover Selector Button (Star Icon) -->
                                <button type="button" @click="coverType = 'existing'; coverIndex = photo.id"
                                    class="absolute top-1 left-1 p-1 bg-black/50 hover:bg-black/80 rounded-none transition-all duration-200 cursor-pointer"
                                    :class="coverType === 'existing' && coverIndex === photo.id ? 'opacity-100 scale-110 border border-amber-400/50' : 'opacity-0 group-hover:opacity-100 scale-100'"
                                    title="Jadikan Sampul Utama">
                                    <i class="fa-solid fa-star text-[10px]" :class="coverType === 'existing' && coverIndex === photo.id ? 'text-amber-400' : 'text-white/85'"></i>
                                </button>

                                <!-- Delete Action Overlay (Trash) -->
                                ...
                                <!-- Cover Active Badge -->
                                <div x-show="coverType === 'existing' && coverIndex === photo.id" 
                    ...
                    <!-- New Uploaded File Previews -->
                    <template x-for="(file, index) in files" :key="'newfile-'+index">
                        <div class="relative aspect-square border border-slate-200 dark:border-zinc-800 p-1 bg-slate-50 dark:bg-zinc-955 flex flex-col justify-between group overflow-hidden">
                            <div class="w-full h-full relative overflow-hidden bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800">
                    ...
                    <!-- New Link Previews -->
                    <template x-for="(link, index) in links" :key="'newlink-'+index">
                        <div x-show="link.url && (link.url.startsWith('http') || link.url.startsWith('https'))" 
                             class="relative aspect-square border border-slate-200 dark:border-zinc-800 p-1 bg-slate-50 dark:bg-zinc-955 flex flex-col justify-between group overflow-hidden">
                            <div class="w-full h-full relative overflow-hidden bg-slate-100 dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800">
                ...
                <div x-show="existingPhotos.length === 0 && files.length === 0 && links.filter(l => l.url && (l.url.startsWith('http') || l.url.startsWith('https'))).length === 0" class="text-xs text-slate-400 dark:text-zinc-555 italic p-4 border border-dashed border-slate-200 dark:border-zinc-800 text-center">
                    Harus menyisakan minimal satu foto atau mengunggah foto baru. Album tidak boleh kosong!
                </div>
            </div>
            ...
            <div class="border-t border-slate-100 dark:border-zinc-800 pt-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-1">
                        ...
                        <input type="text" name="judul" value="{{ old('judul', $galeri->judul) }}" required placeholder="MABIT Ramadhan 1447 H"
                    ...
                    <div class="md:col-span-1">
                        ...
                        <div class="relative" x-data="{ open: false, selected: '{{ old('kategori', $galeri->kategori) }}' }">
                            <button type="button" @click="open = !open" @click.away="open = false"
                                ...
                                <i class="fa-solid fa-chevron-down text-[10px] text-slate-400 dark:text-zinc-555 transition-transform duration-250" :class="open ? 'rotate-180' : ''"></i>
                            </button>
                            ...
                            <div x-show="open" 
                                ...
                                <template x-for="option in ['Belajar', 'Ekskul', 'Fasilitas', 'Event Seru']">
                                    <button type="button" @click="selected = option; open = false"
                                        class="w-full text-left px-4 py-2 text-xs text-slate-755 dark:text-zinc-355 hover:bg-[#4f45b2] hover:text-white dark:hover:bg-[#4f45b2] transition-colors duration-150 flex items-center justify-between cursor-pointer"
                    ...
                    <div class="md:col-span-1">
                        ...
                        <input type="number" name="tahun" value="{{ old('tahun', $galeri->tahun) }}" required min="2000" max="2100"
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3" placeholder="Ceritakan detail kegiatan atau gambar-gambar ini..."
                        class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2] resize-y">{{ old('deskripsi', $galeri->deskripsi) }}</textarea>
                </div>
            </div>
...
<script>
function galeriEditForm() {
    return {
        existingPhotos: {!! json_encode($galeri->photos) !!},
        removedPhotoIds: [],
        files: [],
        rawFiles: [],
        links: [],
        ...
        init() {
            // Find current cover photo
            const cover = this.existingPhotos.find(p => p.is_cover);
            if (cover) {
                this.coverType = 'existing';
                this.coverIndex = cover.id;
            } else {
        ...
        removeExistingPhoto(id, index) {
            this.existingPhotos.splice(index, 1);
            this.removedPhotoIds.push(id);
            if (this.coverType === 'existing' && this.coverIndex === id) {
                this.setDefaultCover();
            }
        },
        ...
        setDefaultCover() {
            if (this.existingPhotos.length > 0) {
                this.coverType = 'existing';
                this.coverIndex = this.existingPhotos[0].id;
