...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (bc) bc.textContent = 'Data Siswa';
});
</script>
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between bg-white dark:bg-zinc-900 p-6 border border-slate-300 dark:border-zinc-800 border-l-4 border-l-[#4f45b2] shadow-[1px_1px_3px_rgba(0,0,0,0.05)]">
        <div>
            <h2 class="text-xl font-bold text-slate-900 dark:text-white leading-tight">Manajemen Siswa</h2>
            <p class="text-xs text-slate-500 dark:text-zinc-400 mt-1 font-mono">Kelola data siswa dan wali murid.</p>
        </div>
            ...
            <div class="flex items-center gap-2 p-4">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           placeholder="Cari siswa — nama, NIS, NISN, atau email..."
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-zinc-800">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-colors">
                        ...
                        <td class="py-3 px-4">
                            <div class="font-bold text-slate-800 dark:text-zinc-200 text-xs">{{ $student->nama }}</div>
                            @if($student->user->email ?? null)
                                <div class="text-[10px] text-slate-400 dark:text-zinc-500 font-mono mt-0.5">{{ $student->user->email }}</div>
                            @endif
                        </td>
                        <td class="py-3 px-4 font-mono text-[11px] text-slate-600 dark:text-zinc-400">
                            <div>NIS: {{ $student->nis ?? '-' }}</div>
                            <div class="text-[10px] text-slate-400">NISN: {{ $student->nisn ?? '-' }}</div>
                        </td>
                        <td class="py-3 px-4 font-mono text-[11px] text-slate-600 dark:text-zinc-400">{{ $student->jenis_kelamin }}</td>
                        <td class="py-3 px-4 font-mono text-[11px] text-slate-600 dark:text-zinc-400">{{ $student->no_telepon ?? '-' }}</td>
                        <td class="py-3 px-4 font-mono text-[11px]">
                            <span class="text-slate-600 dark:text-zinc-400">{{ $student->nama_ayah ?? '-' }}</span>
                            <span class="text-slate-300 dark:text-zinc-600">/</span>
                            <span class="text-slate-600 dark:text-zinc-400">{{ $student->nama_ibu ?? '-' }}</span>
                        </td>
                        ...
                        <td class="py-3 px-4 text-right">
                            <div class="flex items-center justify-end gap-1.5">
                                ...
                                <form method="POST" action="{{ route('admin.students.destroy', $student) }}" class="inline" onsubmit="return confirm('Hapus siswa {{ $student->nama }}? Semua data terkait akan dihapus.')">
                                    @csrf @method('DELETE')
                                    <button type="submit"
                    ...
                    <tr>
                        <td colspan="8" class="py-10 text-center text-slate-400 dark:text-zinc-500">
                            <i class="fa-solid fa-user-graduate text-3xl text-slate-300 dark:text-zinc-600 mb-3 block"></i>
                            <p class="text-sm font-semibold text-slate-600 dark:text-zinc-400 font-mono">Belum ada data siswa.</p>
                            <p class="text-xs text-slate-400 dark:text-zinc-500 mt-1">Gunakan tombol "Tambah Siswa" untuk menambahkan data pertama.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
