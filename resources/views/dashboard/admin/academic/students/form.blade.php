...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (bc) bc.textContent = '{{ $student ? "Edit Siswa" : "Tambah Siswa" }}';
...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4 bg-white dark:bg-zinc-900 p-6 border border-slate-300 dark:border-zinc-800 border-l-4 border-l-[#4f45b2] shadow-[1px_1px_3px_rgba(0,0,0,0.05)]">
        <a href="{{ route('admin.students.index') }}" class="w-8 h-8 flex items-center justify-center bg-slate-100 hover:bg-slate-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 border border-slate-200 dark:border-zinc-700 text-slate-600 dark:text-zinc-400 transition-colors shrink-0">
            <i class="fa-solid fa-arrow-left text-xs"></i>
        </a>
        <div>
            <h2 class="text-xl font-bold text-slate-900 dark:text-white leading-tight">{{ $student ? 'Edit Siswa' : 'Tambah Siswa Baru' }}</h2>
            <p class="text-xs text-slate-500 dark:text-zinc-400 mt-1 font-mono">{{ $student ? 'Perbarui data siswa.' : 'Isi data siswa dengan lengkap.' }}</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ $student ? route('admin.students.update', $student) : route('admin.students.store') }}" enctype="multipart/form-data">
        ...
        <div class="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 border-t-4 border-t-[#4f45b2] shadow-[1px_1px_3px_rgba(0,0,0,0.05)]">
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-6 p-6">
                ...
                <div class="lg:col-span-1">
                    <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-2">Foto Siswa</label>
                    <div id="photoDropzone"
                        ...
                        @if($student && $student->foto)
                            <img id="photoPreview" src="{{ asset('storage/'.$student->foto) }}" alt="{{ $student->nama }}" class="w-full h-full object-cover">
                ...
                <div class="lg:col-span-4 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">NIS</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">NISN</label>
                            <input type="text" name="nisn" value="{{ old('nisn', $student->nisn ?? '') }}"
                                   placeholder="Nomor Induk Siswa Nasional"
                                   class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                            @error('nisn') <p class="text-[10px] text-rose-500 mt-1">{{ $message }}</p> @enderror
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">Password {{ !$student ? '<span class="text-rose-500">*</span>' : '' }}</label>
                            <input type="password" name="password" value=""
                                   placeholder="{{ $student ? 'Kosongkan jika tidak diubah' : 'Minimal 8 karakter' }}"
                                   class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_lahir" value="{{ old('tanggal_lahir', $student?->tanggal_lahir?->format('Y-m-d') ?? '') }}"
                               class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                    </div>
                    <div class="lg:col-span-2">
                        ...
                        <textarea name="alamat" rows="2"
                                  ...
                                  class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]">{{ old('alamat', $student->alamat ?? '') }}</textarea>
                    </div>
                    <div>
                        ...
                        <input type="text" name="no_telepon" value="{{ old('no_telepon', $student->no_telepon ?? '') }}"
        ...
        <div class="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 border-t-4 border-t-amber-600 shadow-[1px_1px_3px_rgba(0,0,0,0.05)]">
            <div class="p-6 border-b border-slate-100 dark:border-zinc-800">
                <h3 class="text-sm font-bold text-slate-800 dark:text-zinc-200 flex items-center gap-2">
                    <i class="fa-solid fa-people-arrows text-amber-600"></i> Data Orang Tua / Wali
                </h3>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">Nama Ayah</label>
                        <input type="text" name="nama_ayah" value="{{ old('nama_ayah', $student->nama_ayah ?? '') }}"
                               placeholder="Nama ayah"
                               class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                    </div>
                    <div>
                        <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">Pekerjaan Ayah</label>
                        <input type="text" name="pekerjaan_ayah" value="{{ old('pekerjaan_ayah', $student->pekerjaan_ayah ?? '') }}"
                               placeholder="Pekerjaan ayah"
                               class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                    </div>
                    <div>
                        <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">Nama Ibu</label>
                        <input type="text" name="nama_ibu" value="{{ old('nama_ibu', $student->nama_ibu ?? '') }}"
                               placeholder="Nama ibu"
                               class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                    </div>
                    <div>
                        <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">Pekerjaan Ibu</label>
                        <input type="text" name="pekerjaan_ibu" value="{{ old('pekerjaan_ibu', $student->pekerjaan_ibu ?? '') }}"
                               placeholder="Pekerjaan ibu"
                               class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">Alamat Orang Tua</label>
                        <textarea name="alamat_orang_tua" rows="2"
                                  placeholder="Alamat orang tua (jika berbeda)"
                                  class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]">{{ old('alamat_orang_tua', $student->alamat_orang_tua ?? '') }}</textarea>
                    </div>
                    <div>
                        <label class="block text-[10px] font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-1.5">No. Telepon Orang Tua</label>
                        <input type="text" name="no_telepon_orang_tua" value="{{ old('no_telepon_orang_tua', $student->no_telepon_orang_tua ?? '') }}"
                               placeholder="08xxxxxxxxxx"
                               class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 border-t-4 border-t-rose-600 shadow-[1px_1px_3px_rgba(0,0,0,0.05)]">
            <div class="p-6 border-b border-slate-100 dark:border-zinc-800">
                <h3 class="text-sm font-bold text-slate-800 dark:text-zinc-200 flex items-center gap-2">
                    <i class="fa-solid fa-graduation-cap text-rose-600"></i> Data Akademik
                </h3>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" name="tanggal_masuk" value="{{ old('tanggal_masuk', $student?->tanggal_masuk?->format('Y-m-d') ?? '') }}"
                               class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 text-slate-700 dark:text-zinc-300 focus:outline-none focus:border-[#4f45b2]"/>
                    </div>
                    <div>
                        ...
                        <select name="status"
                            ...
                            <option value="nonaktif" {{ old('status', $student->status ?? 'aktif') == 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 shadow-[1px_1px_3px_rgba(0,0,0,0.05)] p-6 flex items-center justify-end gap-3">
            ...
            <button type="submit"
               class="py-2.5 px-6 bg-[#4f45b2] hover:bg-[#4f45b2]/90 text-white font-bold text-xs font-mono tracking-wider transition-all inline-flex items-center gap-2">
                <i class="fa-solid fa-floppy-disk"></i> {{ $student ? 'SIMPAN PERUBAHAN' : 'SIMPAN DATA SISWA' }}
            </button>
        </div>
