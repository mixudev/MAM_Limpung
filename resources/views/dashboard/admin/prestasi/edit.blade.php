...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (breadcrumb) {
            breadcrumb.textContent = 'Edit Prestasi';
        }
    });
</script>
...
<div class="max-w-4xl space-y-6">
    ...
    <div class="bg-white dark:bg-zinc-900 p-6 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Ubah Data Prestasi</h1>
            <p class="text-xs text-slate-500 dark:text-zinc-400 mt-1">Ubah rincian informasi prestasi yang telah diraih oleh siswa.</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm p-6">
        <form action="{{ route('admin.prestasi.update', $prestasi->id) }}" method="POST" enctype="multipart/form-data" id="prestasiForm" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="judul" value="{{ old('judul', $prestasi->judul) }}" required placeholder="Contoh: Juara 1 Olimpiade Fisika"
                    ...
                    <div>
                        ...
                        <input type="text" name="peraih" value="{{ old('peraih', $prestasi->peraih) }}" required placeholder="Contoh: Ahmad Fauzan atau Tim Basket Putra"
                            class="w-full px-3 py-2.5 text-sm bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2]" />
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="juara" value="{{ old('juara', $prestasi->juara) }}" placeholder="Contoh: Juara 1, Emas"
                        ...
                        <div>
                            ...
                            <input type="text" name="penyelenggara" value="{{ old('penyelenggara', $prestasi->penyelenggara) }}" placeholder="Contoh: Univ Diponegoro"
                                class="w-full px-3 py-2.5 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2]" />
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="jenis" required
                                ...
                                <option value="non_akademik" {{ old('jenis', $prestasi->jenis) === 'non_akademik' ? 'selected' : '' }}>Non-Akademik</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="tahun" value="{{ old('tahun', $prestasi->tahun) }}" required min="2000" max="2100"
                                class="w-full px-3 py-2.5 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2]" />
                        </div>
                        <div>
                            ...
                            <input type="date" name="tanggal_prestasi" value="{{ old('tanggal_prestasi', $prestasi->tanggal_prestasi?->format('Y-m-d')) }}"
                                class="w-full px-3 py-2.5 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2]" />
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div id="dropzone" class="relative border-2 border-dashed border-slate-200 dark:border-zinc-700 hover:border-[#4f45b2] dark:hover:border-[#4f45b2] bg-slate-50/50 dark:bg-zinc-800/30 p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-all duration-300 rounded-none min-h-[170px]">
                            <!-- Prompt -->
                            <div id="dropzone-prompt" class="{{ $prestasi->foto ? 'hidden' : '' }} space-y-2">
                            ...
                            <!-- Preview -->
                            <div id="dropzone-preview" class="{{ $prestasi->foto ? '' : 'hidden' }} space-y-2 w-full">
                                <div class="relative max-w-[160px] mx-auto border border-slate-200 dark:border-zinc-800 shadow shadow-slate-100 dark:shadow-none overflow-hidden bg-white dark:bg-zinc-900 group">
                                    <img id="preview-image" src="{{ $prestasi->foto ? asset('storage/' . $prestasi->foto) : '' }}" alt="Pratinjau Foto" class="w-full max-h-[110px] object-cover">
                                ...
                                <p class="text-[9px] font-mono text-slate-500 truncate px-2" id="preview-filename">{{ $prestasi->foto ? basename($prestasi->foto) : '' }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="deskripsi" rows="4" placeholder="Masukkan keterangan lebih detail mengenai prestasi yang diraih..."
                            class="w-full px-3 py-2 text-xs bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2] resize-y">{{ old('deskripsi', $prestasi->deskripsi) }}</textarea>
                    </div>

                    <!-- Tampilkan Utama (Featured) Toggle -->
                    <div class="flex items-start gap-3 pt-2">
                        <div class="flex items-center h-5">
                            <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $prestasi->is_featured) ? 'checked' : '' }}
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Check if there is an existing uploaded temp photo (recovery from validation error)
