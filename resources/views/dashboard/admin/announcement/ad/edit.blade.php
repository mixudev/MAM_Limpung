...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (breadcrumb) {
            breadcrumb.textContent = 'Edit Iklan Banner';
        }
    });
</script>
...
<div class="max-w-6xl space-y-6">
    <div class="bg-white dark:bg-zinc-900 p-6 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Edit Iklan Banner</h1>
            <p class="text-xs text-slate-500 dark:text-zinc-400 mt-1">Ubah konfigurasi detail media promosi visual iklan banner.</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm p-6">
        <form action="{{ route('admin.announcements.ads.update', $announceAd) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="flex flex-col">
                    <label class="block text-xs font-mono font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-2">Berkas Gambar Banner Iklan <span class="text-xs font-normal text-slate-400">(Maks 2MB)</span></label>
                    ...
                    <div id="dropzone" 
                        ...
                        <div id="empty-state" class="space-y-4 py-8">
                            ...
                            <div>
                                <p class="text-sm font-bold text-slate-700 dark:text-zinc-300">Tarik & Lepas gambar baru di sini</p>
                ...
                <div class="flex flex-col space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title', $announceAd->title) }}" required placeholder="Contoh: Banner Promosi PPDB 2026"
                    ...
                    <div>
                        ...
                        <input type="url" name="action_url" value="{{ old('action_url', $announceAd->action_url) }}" placeholder="https://example.com/halaman-ppdb"
                            class="w-full px-3 py-2.5 text-sm bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2]" />
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="action_text" value="{{ old('action_text', $announceAd->action_text) }}" placeholder="Contoh: Info Lengkap / Daftar"
                            class="w-full px-3 py-2.5 text-sm bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2]" />
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-200 dark:border-zinc-800">
                ...
                <div>
                    ...
                    <input type="datetime-local" name="start_date" value="{{ $announceAd->start_date ? $announceAd->start_date->format('Y-m-d\TH:i') : '' }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" name="end_date" value="{{ $announceAd->end_date ? $announceAd->end_date->format('Y-m-d\TH:i') : '' }}"
                        class="w-full px-3 py-2.5 text-sm bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 focus:outline-none" />
                </div>

                <!-- Status -->
                <div class="md:col-span-2 flex items-center gap-3 pt-2">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $announceAd->is_active ? 'checked' : '' }}
                        class="w-4 h-4 text-[#4f45b2] border-slate-300 focus:ring-[#4f45b2] rounded-none" />
                    <label for="is_active" class="text-sm font-bold text-slate-700 dark:text-zinc-300 select-none">Aktifkan banner promosi ini</label>
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const initialUrl = "{{ $announceAd->image ? asset('storage/' . $announceAd->image) : '' }}";
        if (initialUrl) {
