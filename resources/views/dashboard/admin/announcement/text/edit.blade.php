...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (breadcrumb) {
            breadcrumb.textContent = 'Edit Teks Berjalan';
        }
    });
</script>
...
<div class="max-w-4xl space-y-6">
    <div class="bg-white dark:bg-zinc-900 p-6 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-xl font-bold text-slate-900 dark:text-white">Edit Teks Berjalan</h1>
            <p class="text-xs text-slate-500 dark:text-zinc-400 mt-1">Ubah kalimat teks berjalan.</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-none shadow-sm p-6">
        <form action="{{ route('admin.announcements.texts.update', $announceText) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $announceText->title) }}" required placeholder="Contoh: Info PPDB 2026 Dibuka"
                ...
                <div>
                    ...
                    <textarea name="content" rows="4" required placeholder="Masukkan pesan pengumuman singkat..."
                        class="w-full px-3 py-2 text-sm bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-none text-slate-700 dark:text-zinc-300 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#4f45b2]/20 focus:border-[#4f45b2]">{{ old('content', $announceText->content) }}</textarea>
                </div>

                <!-- Status -->
                <div class="flex items-center gap-3 pt-4 border-t border-slate-100 dark:border-zinc-800">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ $announceText->is_active ? 'checked' : '' }}
