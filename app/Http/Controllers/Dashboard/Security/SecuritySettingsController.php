...
class SecuritySettingsController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $backupHistory = SecuritySetting::getValue('backup_history', []);
     ...
     */
    public function updateBackupSettings(Request $request): \Illuminate\Http\RedirectResponse
    {
        $request->validate([
     ...
     */
    public function generateKey(): \Illuminate\Http\RedirectResponse
    {
     ...
     */
    public function downloadKey(): \Symfony\Component\HttpFoundation\StreamedResponse|\Illuminate\Http\RedirectResponse
    {
        $backupSettings = SecuritySetting::getValue('backup_settings', []);
        ...
        $content = "=== KUNCI ENKRIPSI BACKUP AMAN MAM LIMPUNG ===\n" .
                   ...
                   "openssl enc -d -aes-256-cbc -salt -in [nama_file].enc -out [nama_file].zip -pass pass:{$decryptedPass}\n";
     ...
     */
    public function downloadBackup(string $filename): BinaryFileResponse|\Illuminate\Http\RedirectResponse
    {
    ...
    public function deleteBackup(string $filename): JsonResponse
    {
        try {
            ...
            // Sync database logs
            $history = SecuritySetting::getValue('backup_history', []);
            $updatedHistory = array_values(array_filter($history, function ($item) use ($safeFilename) {
                return $item['filename'] !== $safeFilename;
            }));
            SecuritySetting::setValue('backup_history', $updatedHistory);
