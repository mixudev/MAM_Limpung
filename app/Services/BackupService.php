...
class BackupService
{
    ...
    public function runBackup(bool $forceFull = false): array
    {
        ...
        try {
            ...
            $this->addHistoryLog($newBackupLog);
    ...
    /**
     * Standard OpenSSL Encryption compatible with openssl CLI.
     */
    public function encryptFile(string $sourcePath, string $destPath, string $password): void
    {
        ...
        $salted = '';
        $dx = '';
        while (strlen($salted) < 48) { // 32 bytes key + 16 bytes IV
    ...
    /**
     * Helper to add log info to SecuritySetting backup_history.
     */
    protected function addHistoryLog(array $logItem): void
    {
        $history = SecuritySetting::getValue('backup_history', []);
        
        // Prepend so that newest is always first
        array_unshift($history, $logItem);
        ...
        SecuritySetting::setValue('backup_history', $history);
    }
